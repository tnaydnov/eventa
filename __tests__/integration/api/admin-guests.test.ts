...
describe('POST /api/admin/events/[eventId]/guests (JSON)', () => {
  function mockEventAndExisting(existing: Array<{ phone: string }> = []) {
    ...
    mockFrom.mockReturnValueOnce({
      select: vi.fn().mockReturnThis(),
      eq: vi.fn().mockReturnThis(),
      single: vi.fn().mockResolvedValue({ data: { id: eventId, wa_messages_enabled: true }, error: null }),
    });
    // Existing phones
    mockFrom.mockReturnValueOnce({
      select: vi.fn().mockReturnThis(),
      eq: vi.fn().mockResolvedValue({ data: existing }),
    });
  }
...
describe('DELETE /api/admin/events/[eventId]/guests', () => {
  it('removes guest phones by IDs', async () => {
    ...
    mockFrom.mockReturnValueOnce({
      ...
      select: vi.fn().mockResolvedValue({ data: [{ id: 'g1' }], error: null }),
    });
    // Update status (count)
    mockFrom.mockReturnValueOnce({
      select: vi.fn().mockReturnThis(),
