...
export async function GET(
  ...
  const denied = await adminGuard(req, 'admin-portal-token-get', RATE_LIMITS.standard);
...
export async function POST(
  ...
  const denied = await adminGuard(req, 'admin-portal-token-post', RATE_LIMITS.standard);
  if (denied) return denied;
