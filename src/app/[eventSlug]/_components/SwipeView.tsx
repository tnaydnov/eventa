...
export default function SwipeView({ participants, eventSlug }: SwipeViewProps) {
  ...
  const handleSwipeRight = useCallback(
    async (id: string) => {
      if (useSwipeStore.getState().isPendingLike(id)) return;
        ...
        if (result) {
          if ('duplicate' in result) {
            ...
            useMatchStore.getState().setPendingMatch({
              ...
              photoUrl: p?.photos?.[0]
                ? getPhotoUrl(p.photos[0].storage_path, { width: 480, height: 640, quality: 80 })
                : null,
            });
          } else {
            toast('💗 לייק נשלח!');
    ...
    [addLiked, removeLiked, toast, participants, startPendingLike, finishPendingLike],
  );
